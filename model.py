...
class FeedforwardDeepNeuralNetwork:
    ...
    def addFeedForwardLayer(self, activationFunction='sigmoid', noOfUnits=10):
        ...
        if activationFunction not in ('sigmoid', 'tanh', 'relu', 'leakyrelu'):
            raise NameError("Invalid activation function name: " + activationFunction)
        else:
    ...
    def train(self, trainSet, trainLabels, noOfIterations=10, learningRate=0.01, validationSet=None,
        ...
        for itrNo in range(noOfIterations):
            ...
            # computing error
            cost = -(1 / self.m) * np.sum((trainLabels * np.log(self.activations["A" + str(self.noOfLayers)]) + (
                        1 - trainLabels) * np.log(1 - self.activations["A" + str(self.noOfLayers)])), axis=1)

            print("Iteration= ", itrNo + 1, ". Cost= ", cost,
                  ". Train Accuracy= {}%".format(100 - np.mean(np.abs(predictions - trainLabels)) * 100), '. ', sep='',
                  end='')
